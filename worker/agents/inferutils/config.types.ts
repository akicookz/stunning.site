...
export const RegularModels: AIModels[] = Object.values(MODELS_MASTER)
	.filter(
		(entry) =>
			entry.config.size === ModelSize.REGULAR ||
			entry.config.size === ModelSize.LITE,
	)
	.map((entry) => entry.id);

export const AllModels: AIModels[] = Object.values(MODELS_MASTER).map(
	(entry) => entry.id,
);
...
export function credentialsToRuntimeOverrides(
	...
	return {
		...(hasKeys ? { userApiKeys } : {}),
		...(credentials.aiGateway
			? { aiGatewayOverride: credentials.aiGateway }
			: {}),
	};
}
